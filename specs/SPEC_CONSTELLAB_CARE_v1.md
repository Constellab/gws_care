...
4. Effectuer la "Validation Labo / Employé" (résultats saisis et vérifiés)
5. Effectuer la "Validation Labo / Campagne" une fois tous les résultats saisis (données verrouillées)
...
- Validation labo (employé et campagne)

**Accès interdits**
- Validation médicale (réservée aux médecins)
- Modification des données après validation labo
- Données de facturation
...
2. Interpréter les résultats d'analyses après validation labo
3. Remplir la case "Interprétation du médecin PSC"
...
- Notification dès que la validation labo d'une campagne est complète
...
```
Brouillon → Validée → En cours → Labo terminé → Médecin PSC validé → Médecin Entreprise validé → Archivée
```

| Statut | Déclencheur | Acteur |
|---|---|---|
| Brouillon | Création | Opérateur Siège |
| Validée | Validation de la campagne | Médecin PSC ou Admin |
| En cours | Date de début atteinte / début terrain | Opérateur Terrain |
| Labo terminé | Validation Labo / Campagne | Opérateur Siège |
| Médecin PSC validé | Validation Médecin / Campagne | Médecin PSC |
...
| `status` | Enum | ✅ | Voir cycle de vie |
| `labo_validated_by` | FK → User | ⬜ Optionnel | |
| `labo_validated_at` | DateTime | ⬜ Optionnel | |
| `doctor_psc_validated_by` | FK → User | ⬜ Optionnel | |
| `doctor_psc_validated_at` | DateTime | ⬜ Optionnel | |
| `doctor_company_validated_by` | FK → User | ⬜ Optionnel | |
| `doctor_company_validated_at` | DateTime | ⬜ Optionnel | |
| `doctor_psc_interpretation` | Text | ⬜ Optionnel | |
...
```
En attente → Terrain terminé → Résultats saisis → Labo validé → Médecin PSC validé → Médecin Entreprise validé
```
...
**Règles métier**
- Après "Validation Labo/Employé", les données sont verrouillées pour modification
- Après "Validation Médecin PSC/Employé", le dossier ne peut plus être modifié
- Le médecin PSC peut modifier les appréciations automatiques (seuils) avant validation
...
- Validation labo terminée (→ Médecin PSC)
- Campagne interprétée par PSC (→ Médecin Entreprise)
...
- Validation Labo / Employé et Labo / Campagne
...
| OS-08 | Opérateur Siège | valider un dossier patient (Validation Labo / Employé) | verrouiller les données saisies |
| OS-09 | Opérateur Siège | valider une campagne entière (Validation Labo / Campagne) | notifier le médecin PSC |
...
### 3.4 Interface Médecin PSC

| Attribut | Détail |
|---|---|
| **Utilisateurs cibles** | Médecin PSC |
| **Canal d'accès** | Web desktop |
| **Langue** | Français |
...
- Saisie de l'interprétation médicale PSC
...
| ID | En tant que | Je veux | Afin de |
|---|---|---|---|
| MP-01 | Médecin PSC | recevoir une notification quand une campagne est prête pour interprétation | savoir quand commencer |
| MP-02 | Médecin PSC | voir la liste de tous les dossiers à interpréter | m'organiser dans mon travail |
| MP-03 | Médecin PSC | accéder au dossier complet d'un patient (examens, résultats, appréciations) | analyser ses résultats |
| MP-04 | Médecin PSC | voir l'historique médical du patient sur ses campagnes précédentes | détecter des évolutions |
| MP-05 | Médecin PSC | voir les appréciations automatiques (Normal/Haut/Bas/Critique) avec codes couleur | repérer rapidement les anomalies |
| MP-06 | Médecin PSC | modifier l'appréciation automatique d'un examen | corriger les calculs si besoin |
| MP-07 | Médecin PSC | saisir mon interprétation médicale pour chaque dossier | documenter mon analyse |
| MP-08 | Médecin PSC | valider un dossier patient (Validation Médecin / Employé) | verrouiller le dossier |
| MP-09 | Médecin PSC | valider l'ensemble d'une campagne (Validation Médecin / Campagne) | notifier le médecin entreprise |
| MP-10 | Médecin PSC | visualiser les images d'examens (radio, ECG, spiro) | interpréter les examens imagerie |
| MP-11 | Médecin PSC | filtrer les dossiers par statut (à interpréter / validé) | me concentrer sur ce qui reste à faire |
| MP-12 | Médecin PSC | valider (ou refuser) une campagne avant son démarrage | vérifier que les examens prévus sont corrects |
...
- Réception de notification : campagne PSC interprétée et disponible
- Liste des dossiers à valider (filtré : son entreprise uniquement)
- Consultation des résultats + interprétation PSC
- Saisie de l'interprétation médecin entreprise
...
**Besoins UX**
- Interface similaire au médecin PSC mais scoped sur une entreprise
...
| ME-03 | Médecin Entreprise | consulter les résultats et l'interprétation du médecin PSC | prendre connaissance des analyses |
...
| Fonctionnalité | Super Admin | OS | OT | Médecin PSC | Médecin Entreprise | RH Entreprise | Patient |
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
...
| Validation Labo | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ |
| Interprétation médicale PSC | ✅ | ❌ | ❌ | ✅ | ❌ | ❌ | ❌ |
| Validation Médecin PSC | ✅ | ❌ | ❌ | ✅ | ❌ | ❌ | ❌ |
