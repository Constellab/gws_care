"""DB migration v0.68.13 — Remove test campaign "PRG-JA5KVBFG" and test
private consultation "VIS-076BCXJZ".

One-off cleanup of two unrelated test records:
  - Campaign PRG-JA5KVBFG (tes_ssuub), with its cascade-linked visits, exam
    configs, doctor assignments, and campaign-linked Exam records (matched
    by the "CAMP:{campaign_id}|" reason_for_visit marker, rather than a
    hardcoded exam ID list). Same explicit-delete-order pattern as
    migration_65.py/67.py/68.py/69.py/71.py/72.py/73.py/74.py/76.py.
  - Private consultation Visit VIS-076BCXJZ (campaign_id is null for these),
...
@brick_migration(
    "0.68.13",
    short_description="Remove test campaign PRG-JA5KVBFG and test consultation VIS-076BCXJZ",
    db_manager=CareDbManager.get_instance(),
    authenticate_sys_user=False,
)
class Migration770(BrickMigration):
    @classmethod
