...
class Exam(ModelWithUser):
    ...
    # Optional link to ExamTypeModel for parameterized exams (Référentiel Examen)
    ...
    weight: float = FloatField(null=True)
    height: float = FloatField(null=True)
    bmi: float = FloatField(null=True)
    blood_pressure: str = CharField(max_length=50, null=True)
    heart_rate: float = FloatField(null=True)
    temperature: float = FloatField(null=True)
    lab_results: list = JSONField(null=True)
    requested_param_ids: list = JSONField(null=True)
