...
class ExamFileService:
    ...
    @classmethod
    def _register_as_gws_resource(cls, temp_path: str, original_name: str) -> str | None:
        """Move a file from `temp_path` into the gws_core file store and register
        it as a ResourceModel(UPLOADED). Returns the resource_model id string, or
        None if registration fails (non-fatal).
        """
        try:
            ...
            gws_file.set_name(original_name)
            resource_model = ResourceModel.save_from_resource(gws_file, origin=ResourceOrigin.UPLOADED)
            return str(resource_model.id)
        except Exception as err:
            from gws_core.core.utils.logger import Logger
            Logger.error(f"ExamFileService: could not register file as gws resource: {err}")
            return None
    ...
    def create_file(
        ...
        """Save file bytes to the Reflex upload dir, register as gws_core Resource,
        and create an ExamFile record.
        """
        import uuid

        from gws_core import Settings

        # 1. Write to a unique name in Reflex upload dir (for in-app rx.get_upload_url)
        ...
        # 2. Write to a gws temp dir and register as a gws_core Resource
        temp_dir = Settings.make_temp_dir()
    ...
    def delete_file(cls, file_id: str) -> None:
        ...
        # Remove the gws_core Resource (cascades file store deletion)
        if ef.resource_id:
            try:
                from gws_core.resource.resource_model import ResourceModel
                resource_model = ResourceModel.get_by_id(ef.resource_id)
                ...
                from gws_core.core.utils.logger import Logger
                Logger.error(f"ExamFileService: could not delete gws resource {ef.resource_id}: {err}")

        # Remove from Reflex upload dir
        rx_path = rx.get_upload_dir() / ef.stored_filename
        if rx_path.exists():
            try:
                os.remove(rx_path)
            except OSError:
