...
```gws_care``` is a [Constellab](https://constellab.io) library (called bricks) developped by [Gencovery](https://gencovery.com/). GWS stands for Gencovery Web Services.
...
Gencovery brick to manage occupational health checkups (bilans médicaux en entreprise) for PS CONSULTING, as a database-backed patient and company management system with a Reflex web application.
- Manage client companies and their patients through a database-backed CRUD system
- Auto-generate unique patient numbers (`PAT-XXXXXXXX`) on patient creation
- Sync gws_core users into a local user table so patient/company records can track who created or last modified them
- Browse and search patients and companies through a dedicated Reflex web application (list, detail, and search pages)
- Serve as an integration point for `gws_care_billing`, a separate brick handling invoicing

## 📄Documentation

📄 For `gws_care` brick documentation, click [here](https://constellab.community/bricks/gws_care/latest/doc/getting-started)
...
The `gws_care` brick requires the `gws_core` brick.
...
▶️ To run the Care Reflex app in dev mode:
...
```gws_care``` is completely free and open-source and licensed under the [GNU General Public License v3.0](https://www.gnu.org/licenses/gpl-3.0.en.html).
